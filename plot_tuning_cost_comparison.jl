...
function latter_half_average(x)
	y = zero(x)
	avg = 0.0
	ir = 0
	il = 1
...
function main()
	...
	for slice_type in [:stepping, :lazydoubling]
		p1 = plot()
		p2 = plot(xlabel="Iteration", xticks= 10.0 .^[0,1,2,3,4,5], legend=nothing)
		...
		for (target, sample_target) in [(logpdf_uniform, sample_uniform), (logpdf_normal, sample_normal), (logpdf_laplace, sample_laplace), (logpdf_cauchy, sample_cauchy)]
			...
			for j=1:size(constant_costs,1)
				plot!(p2, 1:T, latter_half_average(oneshot_costs[j,:]), linestyle=:dash, alpha=i/4, linewidth=0.5, ylims=ylims)
				plot!(p2, 1:T, latter_half_average(gradient_costs[j,:]), linestyle=:dot, alpha=i/4, linewidth=0.5)
			end
			i += 1
		end
 		hline!(p1, [optimal_cost], color=:grey, linestyle=:dash)
 		hline!(p2, [optimal_cost], color=:grey, linestyle=:dash)

		# Color legend
		plot!(p1, [NaN], [NaN], alpha=1/4, linestyle=:solid, label="Uniform")
		plot!(p1, [NaN], [NaN], alpha=2/4, linestyle=:solid, label="Normal")
		plot!(p1, [NaN], [NaN], alpha=3/4, linestyle=:solid, label="Laplace")
		plot!(p1, [NaN], [NaN], alpha=4/4, linestyle=:solid, label="Cauchy")
		plot!(p2, [NaN], [NaN], alpha=1/4, linestyle=:solid, label="Uniform")
		plot!(p2, [NaN], [NaN], alpha=2/4, linestyle=:solid, label="Normal")
		plot!(p2, [NaN], [NaN], alpha=3/4, linestyle=:solid, label="Laplace")
		plot!(p2, [NaN], [NaN], alpha=4/4, linestyle=:solid, label="Cauchy")
		...
		plot!(p2, [NaN], [NaN], color=:black, linewidth=1, linestyle=:dash,  label="One Shot")
		plot!(p2, [NaN], [NaN], color=:black, linewidth=1, linestyle=:dot,  label="Gradient")

		# optimal legend
		plot!(p1, [NaN], [NaN], color=:grey, linestyle=:dash, linewidth=1, label="Optimal")
		plot!(p2, [NaN], [NaN], color=:grey, linestyle=:dash, linewidth=1, label="Optimal")

		savefig(p1, "tuning_cost_comparison_$(string(slice_type)).png")
		savefig(p2, "tuning_cost_traces_$(string(slice_type)).png")
